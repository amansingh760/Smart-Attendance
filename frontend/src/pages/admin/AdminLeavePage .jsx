    ...
    <div style={{ padding: 24 }}>
      <div className="flex-between mb-24">
        <div>
          ...
          <p style={{ color: 'var(--text2)', fontSize: 14 }}>Review and manage employee leave requests</p>
        </div>
      </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display:'flex', gap:20, marginBottom:8, flexWrap:'wrap' }}>
                        <div style={{ fontSize:13 }}><span style={{ color:'var(--text3)' }}>Dates: </span><span style={{ fontFamily:'var(--mono)', color:'var(--text)' }}>{l.fromDate === l.toDate ? l.fromDate : `${l.fromDate} → ${l.toDate}`}</span></div>
                        <div style={{ fontSize:13 }}><span style={{ color:'var(--text3)' }}>Applied: </span><span style={{ color:'var(--text)' }}>{new Date(l.appliedAt).toLocaleDateString('en-IN')}</span></div>
                      </div>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  {LEAVE_TYPES.map(t => <th key={t} style={{ textAlign:'center', fontSize:10 }}>{TYPE_ICON[t]} {TYPE_LABEL[t]}</th>)}
      ...
      {reviewModal && (
        <Modal title={`Review Leave — ${reviewModal.userName}`} onClose={() => { setReviewModal(null); setAdminNote(''); }}>
          ...
          <div className="form-group">
            ...
            <textarea className="form-textarea" placeholder="Optional note to employee…" value={adminNote}
